...
And one question this project leaves open. If describing a scene stays easy
while counting its events gets harder, what do caption metrics actually
measure: scene understanding, or scene summarization?

My own data points to summarization. Every system gained on the event-rich
clips. Even the simple tagger. So the metric rewards having more to
summarize, not understanding more. Which is exactly why the two levels belong
together: your event level and my caption level, on the same recordings.
